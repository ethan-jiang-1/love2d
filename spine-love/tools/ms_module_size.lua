...
function _M.all_module_data_size( )
    local all_by_name = {}
    local all_by_size = {}
    local total = 0
...
local function _cmp_otbl(obj1, obj2)
    return obj1.size < obj2.size
end

function _M.module_data_top_size()
    if _G.ref_find == nil then
        print("Please specify ref at _G.ref_find")
        return nil
    end
    ...
    local otbl = {}
    for key, val in pairs(_G.ref_find) do 
        local size
        if type(val) == "table" then
            size = _M._sum_table(val)
        else
            size = _G.debug.getsize(val)
        end
        table.insert(otbl, {name=key, type=type(val), size=size})
    end
    table.sort(otbl, _cmp_otbl)
    for _, obj in pairs(otbl) do 
        print(obj.size, obj.type, obj.name)
    end
    return _G.debug.getsize(_G.ref_find)    
end
